> Archived reviewed version, before the user clarified root business-plan ownership and single-idea execution. Relative links rebased for this archive location.
...
Independent final review: **A 85/100, B 87/100; both qualify as design options.** The judge stopped after three of the permitted five rounds. These are weighted design judgments, not measured usability or implementation results. [Final judgment](../round-3/judge.md).
...
projects/<topic-or-venture>/
  ...
  strategy/<family>/<scope>/current.md
  history/
...
projects/<topic-or-venture>/
  README.md                           # current overview/comparison
  project-manifest.json                # discovery/venture context and tracks
  ...
  cases/<case>/
    ...
    strategy/<family>/current.md       # only as actual work requires
    history/snapshots/
  ...
  strategy/...                        # shared constraints and comparisons
  history/
...
**Distinctive property:** each case owns its state and its future research/strategy files. This is why B is the stronger fit for the workflow described here. It can use the same single project timeline as A; logging policy is not dictated by physical storage.
...
| “Build for the selected idea” | Use that scope's prerequisites, authorization and immutable handoff; another scope's pass does not qualify it. |
